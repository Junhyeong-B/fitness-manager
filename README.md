# 😎 Fitness Manager
- 운동 정보, 운동 기록, 식단 등의 정보를 확인할 수 있는 페이지 제작 개인 프로젝트 입니다.
- 최초 프로젝트 작성 시 파일 구조가 단순했고, 한 페이지 당 HTML, CSS, Javascript 1개씩 존재하는 등 완성도가 낮았습니다.(21 06.04~08.03 진행)
- 프로그래머스 웹 데브코스 1기(21 07.31~12.23) 교육 수강 후 해당 프로젝트를 React, SPA 형태로 리팩토링을 진행합니다.(21.01.05 ~ 현재)
 ...
 * 버전 0.5.7.2 : Nutrition 페이지에서 아이콘 이미지를 드래그하여 (+) 이미지가 있는 곳에 놓으면 이동하도록 수정(이미지 정렬, 다음 아이콘 놓았을 때 (+)에 있었던 이미지 원복 등 기능 추가 예정)
 * 버전 0.5.7.3 : Nutrition 페이지에서 아이콘 이미지를 드랍한 후 다음 이미지를 드랍했을 때 기존 위치로 이미지 원복하는 기능 추가, 아이콘 이미지 정렬되도록 수정
 * 버전 0.5.7.4 : Nutrition 페이지에서 아이콘 이미지를 (+)에 이동 시 아래쪽에 정보 나타나도록 기능 추가(내용은 추가 업데이트 예정)
 * 버전 0.5.7.5 : Nutrition html, css, js 파일 불필요한 코멘트 삭제, main html에서 Nutrition으로 넘어가는 link 수정(nutrition_ver2.html)
